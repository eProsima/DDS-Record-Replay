...
public:
    ...
    CPP_UTILS_DllAPI Timer() noexcept;

    //! Reset timer initial time to the moment \c reset is called.
    CPP_UTILS_DllAPI virtual void reset() noexcept;

    //! Elapsed time in milliseconds since beggining or last reset
    CPP_UTILS_DllAPI virtual double elapsed() const noexcept;
     ...
     */
    CPP_UTILS_DllAPI Duration_ms elapsed_ms() const noexcept;
