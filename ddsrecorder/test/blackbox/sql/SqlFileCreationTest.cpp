...
protected:
    ...
    /**
     * @brief Wait until the recorder has stopped adding rows to the output it still holds open.
     *
     * Returns as soon as \c expected rows are present, which is the common case and costs nothing.
     * Tests that deliberately record fewer downsampling, content filters, a stopped or paused
     * recorder never reach that count, so fall back to waiting for the row count to stop growing.
     *
     * Reading the database while the recorder is writing it is safe: it is opened in WAL mode, which
     * allows concurrent readers.
     */
    void wait_for_recording_to_drain_(
            const std::string& file_name,
            const std::size_t expected) override
    {
        // The recorder writes to a temporary file and renames it on close
        const auto tmp_path = (std::filesystem::current_path() / (file_name + ".db")).string() + ".tmp~";

        constexpr auto POLL_PERIOD = std::chrono::milliseconds(50);
        // The handler writes in batches of buffer_size, so the count can legitimately stall between
        // flushes. Keep this above the time it takes to publish one batch
        constexpr auto STABLE_PERIOD = std::chrono::milliseconds(1500);
        const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(30);

        auto last_count = recorded_messages_(tmp_path);
        ...
        while (std::chrono::steady_clock::now() < deadline)
        {
            ...
            {
                // Everything that was sent has been written
                return;
            }

            std::this_thread::sleep_for(POLL_PERIOD);

            const auto count = recorded_messages_(tmp_path);
            ...
            {
                // The recorder is no longer writing, so it is not going to reach expected
                return;
            }
        }
    }

    //! @return number of rows in the Messages table, or 0 if the file cannot be read yet
    std::size_t recorded_messages_(
            const std::string& file_path)
    {
        ...
        if (sqlite3_prepare_v2(database, "SELECT COUNT(*) FROM Messages;", -1, &stmt, nullptr) == SQLITE_OK
