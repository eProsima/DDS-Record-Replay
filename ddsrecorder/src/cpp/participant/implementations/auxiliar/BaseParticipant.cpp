...
{
    logDebug(DDSRECORDER_TRACK, "Creating Participant " << *this << ".");
}

BaseParticipant::~BaseParticipant()
{
    logDebug(DDSRECORDER_TRACK, "Destroying Participant " << *this << ".");

    if (!writers_.empty())
    {
        logDevError(DDSRECORDER_BASEPARTICIPANT, "Deleting Participant " << id() << " with still alive writers");
        writers_.clear();
    }

    if (!readers_.empty())
    {
        logDevError(DDSRECORDER_BASEPARTICIPANT, "Deleting Participant " << id() << " with still alive readers");
        readers_.clear();
    }

    logDebug(DDSRECORDER_TRACK, "Participant " << *this << " destroyed.");
}
        ...
        types::DdsTopic topic)
{
    ...
    logInfo(DDSRECORDER_BASEPARTICIPANT, "Created writer in Participant " << id() << " for topic " << topic);
        ...
        types::DdsTopic topic)
{
    ...
    logInfo(DDSRECORDER_BASEPARTICIPANT, "Created reader in Participant " << id() << " for topic " << topic);
