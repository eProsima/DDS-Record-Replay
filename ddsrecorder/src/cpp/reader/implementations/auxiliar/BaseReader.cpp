...
const std::function<void()> BaseReader::DEFAULT_ON_DATA_AVAILABLE_CALLBACK =
        []()
        {
            logDevError(DDSRECORDER_READER, "Calling unset callback");
        };
...
BaseReader::BaseReader(
    ...
    , enabled_(false)
{
    logDebug(DDSRECORDER_BASEREADER, "Creating Reader " << *this << ".");
}
        ...
        std::function<void()> on_data_available_lambda) noexcept
{
    ...
    {
        logDevError(DDSRECORDER_BASEREADER, "Changing on_data_available callback for Reader in topic " <<
...
void BaseReader::unset_on_data_available_callback() noexcept
{
    ...
    if (!on_data_available_lambda_set_)
    {
        logDevError(DDSRECORDER_BASEREADER, "Unsetting a non set on_data_available callback for Reader in topic " <<
                topic_ << " in Participant " << participant_id_);
    }
        ...
        std::unique_ptr<DataReceived>& data) noexcept
{
    ...
    {
        logDevError(DDSRECORDER_BASEREADER, "Attempt to take data from disabled Reader in topic " <<
                topic_ << " in Participant " << participant_id_);
...
void BaseReader::on_data_available_() const noexcept
{
    ...
    {
        logDevError(DDSRECORDER_BASEREADER, "Calling not set on_data_available callback for Reader in topic " <<
                topic_ << " in Participant " << participant_id_);
    }
}
