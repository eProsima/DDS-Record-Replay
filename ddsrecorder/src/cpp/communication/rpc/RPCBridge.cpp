...
RPCBridge::RPCBridge(
    ...
    , init_(false)
{
    logDebug(DDSRECORDER_RPCBRIDGE, "Creating RPCBridge " << *this << ".");

    logDebug(DDSRECORDER_RPCBRIDGE, "RPCBridge " << *this << " created.");
}

RPCBridge::~RPCBridge()
{
    logDebug(DDSRECORDER_RPCBRIDGE, "Destroying RPCBridge " << *this << ".");
    ...
    logDebug(DDSRECORDER_RPCBRIDGE, "RPCBridge " << *this << " destroyed.");
}

void RPCBridge::init_nts_()
{
    logInfo(DDSRECORDER_RPCBRIDGE, "Creating endpoints in RPCBridge for service " << topic_ << ".");
...
void RPCBridge::enable() noexcept
{
    ...
    {
        logInfo(DDSRECORDER_RPCBRIDGE, "Enabling RPCBridge for service " << topic_ << ".");
        ...
        if (!init_)
        {
            ...
            {
                logError(DDSRECORDER_RPCBRIDGE,
...
void RPCBridge::disable() noexcept
{
    ...
    if (enabled_)
    {
        logInfo(DDSRECORDER_RPCBRIDGE, "Disabling RPCBridge for service " << topic_ << ".");
        ...
        const Guid& reader_guid) noexcept
{
    ...
    {
        logDebug(
            DDSRECORDER_RPCBRIDGE, "RPCBridge " << *this <<
        ...
        if (!task.first)
        {
            ...
            logDebug(DDSRECORDER_RPCBRIDGE, "RPCBridge " << *this <<
    ...
    {
        {
            ...
            if (!enabled_ || !(reader->get_unread_count() > 0))
            {
                if (!enabled_)
                {
                    logDebug(DDSRECORDER_RPCBRIDGE,
        ...
        {
            // Error reading data
            logWarning(DDSRECORDER_RPCBRIDGE,
        ...
        if (RPCTopic::is_request_topic(reader->topic()))
        {
            ...
            else
            {
                for (auto& service_registry : service_registries_)
                {
                    ...
                    if (!ret)
                    {
                        logWarning(DDSRECORDER_RPCBRIDGE, "Error writting request in RPCBridge for service "
        ...
        else if (RPCTopic::is_reply_topic(reader->topic()))
        {
            ...
            // A Server could be answering a different client in this same DDS Recorder or a remote client
            ...
            else
            {
                ...
                if (registry_entry.first.is_valid())
                {
                    ...
                    if (!ret)
                    {
                        logWarning(DDSRECORDER_RPCBRIDGE, "Error writting reply in RPCBridge for service "
