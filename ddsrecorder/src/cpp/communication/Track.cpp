...
{
    logDebug(DDSRECORDER_TRACK, "Creating Track " << *this << ".");
    ...
    logDebug(DDSRECORDER_TRACK, "Track " << *this << " created.");
}

Track::~Track()
{
    logDebug(DDSRECORDER_TRACK, "Destroying Track " << *this << ".");
    ...
    logDebug(DDSRECORDER_TRACK, "Track " << *this << " destroyed.");
}
...
void Track::enable() noexcept
{
    ...
    if (!enabled_)
    {
        logInfo(DDSRECORDER_TRACK, "Enabling Track " << reader_participant_id_ << " for topic " << topic_ << ".");
        enabled_ = true;
...
void Track::disable() noexcept
{
    ...
    if (enabled_)
    {
        logInfo(DDSRECORDER_TRACK, "Disabling Track " << reader_participant_id_ << " for topic " << topic_ << ".");
...
void Track::data_available_() noexcept
{
    ...
    if (enabled_)
    {
        logDebug(DDSRECORDER_TRACK, "Track " << *this << " has data ready to be sent.");
        ...
        if (previous_status == DataAvailableStatus::no_more_data)
        {
            ...
            logDebug(DDSRECORDER_TRACK, "Track " << *this << " send callback to queue.");
        }
    }
}
...
void Track::transmit_() noexcept
{
    ...
    while (should_transmit_())
    {
        ...
        {
            // Error reading data
            logWarning(DDSRECORDER_TRACK, "Error taking data in Track " << topic_ << ". Error code " << ret
                                                                      << ". Skipping data and continue.");
            continue;
        }

        logDebug(DDSRECORDER_TRACK,
        ...
        for (auto& writer_it : writers_)
        {
            ...
            if (!ret)
            {
                logWarning(DDSRECORDER_TRACK, "Error writting data in Track " << topic_ << ". Error code "
                                                                            << ret <<
