...
DiscoveryDatabase::DiscoveryDatabase() noexcept
    ...
    , enabled_(false)
{
    logDebug(DDSRECORDER_DISCOVERY_DATABASE, "Creating queue processing thread.");
}

DiscoveryDatabase::~DiscoveryDatabase()
{
    logDebug(DDSRECORDER_DISCOVERY_DATABASE, "Destroying Discovery Database.");
...
void DiscoveryDatabase::start() noexcept
{
    if (!enabled_.load())
    {
        ...
        enabled_.store(true);
        logDebug(DDSRECORDER_DISCOVERY_DATABASE, "Creating queue processing thread routine.");
    }
    else
    {
        logDebug(DDSRECORDER_DISCOVERY_DATABASE, "Processing thread routine already started.");
    }
}
...
void DiscoveryDatabase::stop() noexcept
{
    if (enabled_.load())
    {
        ...
        logDebug(DDSRECORDER_DISCOVERY_DATABASE, "Waiting for queue processing thread to finish.");
        enabled_.store(false);
        queue_processing_thread_.join();
    }
    else
    {
        logDebug(DDSRECORDER_DISCOVERY_DATABASE, "Processing thread routine already stopped.");
    }
}
...
{
    {
        ...
        if (it != entities_.end())
        {
            ...
            else
            {
                ...
                logInfo(DDSRECORDER_DISCOVERY_DATABASE,
        ...
        {
            logInfo(DDSRECORDER_DISCOVERY_DATABASE, "Inserting a new discovered Endpoint " << new_endpoint << ".");
...
{
    {
        ...
        logInfo(DDSRECORDER_DISCOVERY_DATABASE, "Erasing Endpoint " << endpoint_to_erase << ".");
...
void DiscoveryDatabase::process_queue_() noexcept
{
    ...
    while (!entities_to_process_.Empty())
    {
        ...
        {
            logDevError(DDSRECORDER_DISCOVERY_DATABASE,
