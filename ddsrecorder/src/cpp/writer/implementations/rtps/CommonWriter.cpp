...
CommonWriter::~CommonWriter()
{
    ...
    logInfo(DDSRECORDER_RTPS_COMMONWRITER, "Deleting CommonWriter created in Participant " <<
        ...
        fastrtps::rtps::MatchingInfo& info) noexcept
{
    if (!come_from_this_participant_(info.remoteEndpointGuid))
    {
        ...
        {
            logInfo(DDSRECORDER_RTPS_COMMONWRITER_LISTENER,
        ...
        std::unique_ptr<DataReceived>& data) noexcept
{
    ...
    logDebug(DDSRECORDER_RTPS_COMMONWRITER,
    ...
    if (!ret)
    {
        logError(DDSRECORDER_RTPS_COMMONWRITER, "Error setting change to send.");
        return ret;
    }
        ...
        std::unique_ptr<types::DataReceived>& data) const noexcept
{
    ...
    if (data->payload.length > 0)
    {
        ...
        {
            logDevError(DDSRECORDER_RTPS_COMMONWRITER, "Error getting Payload.");
            return utils::ReturnCode::RETCODE_ERROR;
        }
    }
        ...
        const utils::PoolConfiguration& pool_configuration)
{
    ...
    if (repeater_)
    {
        logDebug(DDSRECORDER_RTPS_COMMONWRITER, "CommonWriter created with repeater filter");
