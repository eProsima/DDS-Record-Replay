        ...
        utils::Formatter& error_msg) const noexcept
{
    ...
    for (std::shared_ptr<DdsFilterTopic> topic : allowlist)
    {
        if (!topic)
        {
            logDevError(DDSRECORDER_CONFIGURATION, "Invalid ptr in allowlist topics.");
    ...
    for (std::shared_ptr<DdsFilterTopic> topic : blocklist)
    {
        if (!topic)
        {
            logDevError(DDSRECORDER_CONFIGURATION, "Invalid ptr in blocklist topics.");
    ...
    for (std::shared_ptr<DdsTopic> topic : builtin_topics)
    {
        if (!topic)
        {
            logDevError(DDSRECORDER_CONFIGURATION, "Invalid ptr in builtin topics.");
