...
PayloadPool::~PayloadPool()
{
    ...
    {
        logDevError(DDSRECORDER_PAYLOADPOOL, "Removing non Consistent PayloadPool.");
    }
    else if (reserve_count_ != release_count_)
    {
        logDevError(DDSRECORDER_PAYLOADPOOL,
                "From " << reserve_count_ << " payloads reserved only " << release_count_ << " has been released.");
    }
    else
    {
        logInfo(DDSRECORDER_PAYLOADPOOL,
        ...
        eprosima::fastrtps::rtps::CacheChange_t& cache_change)
{
    ...
    {
        logDevError(DDSRECORDER_PAYLOADPOOL, "Error occurred while creating payload.")
        return false;
    }
}
        ...
        eprosima::fastrtps::rtps::CacheChange_t& cache_change)
{
    ...
    {
        logDevError(DDSRECORDER_PAYLOADPOOL, "Error occurred while referencing payload.")
        return false;
    }
}
        ...
        fastrtps::rtps::CacheChange_t& cache_change)
{
    if (cache_change.payload_owner() == this)
    {
        ...
        {
            logDevError(DDSRECORDER_PAYLOADPOOL, "Error occurred while releasing payload.")
            return false;
        }
    }
    else
    {
        logError(DDSRECORDER_PAYLOADPOOL, "Trying to remove a cache change in an incorrect pool.")
...
void PayloadPool::add_release_payload_()
{
    ...
    {
        logError(DDSRECORDER_PAYLOADPOOL,
