...
ParticipantsDatabase::~ParticipantsDatabase()
{
    ...
    {
        logDevError(DDSRECORDER_PARTICIPANT_DATABASE, "Erasing Participant Database with still Participants in it");
    }
}
        ...
        std::shared_ptr<IParticipant> participant)
{
    ...
    {
        logInfo(DDSRECORDER_PARTICIPANT_DATABASE, "Inserting a new Participant " << id);
    }
        ...
        const ParticipantId& id) noexcept
{
    ...
    logInfo(DDSRECORDER_PARTICIPANT_DATABASE, "Poping Participant " << participant_to_erase->id());
