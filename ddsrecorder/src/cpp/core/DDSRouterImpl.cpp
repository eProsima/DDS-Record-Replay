...
{
    logDebug(DDSRECORDER, "Creating DDS Recorder.");
    ...
    if (!configuration_.is_valid(error_msg))
    {
        throw utils::ConfigurationException(
                  utils::Formatter() <<
                      "Configuration for DDS Recorder is invalid: " << error_msg);
    }
    ...
    logDebug(DDSRECORDER, "DDS Recorder created.");
}

DDSRouterImpl::~DDSRouterImpl()
{
    logDebug(DDSRECORDER, "Destroying DDS Recorder.");
    ...
    while (!participants_database_->empty())
    {
        ...
        if (!participant)
        {
            logDevError(DDSRECORDER, "Error poping participant from database.");
        }
        else
        {
    ...
    logDebug(DDSRECORDER, "DDS Recorder destroyed.");
}
        ...
        const configuration::DDSRouterReloadConfiguration& new_configuration)
{
    ...
    if (!new_configuration.is_valid(error_msg))
    {
        throw utils::ConfigurationException(
                  utils::Formatter() <<
                      "Configuration for Reload DDS Recorder is invalid: " << error_msg);
    }
    ...
    if (enabled_.load())
    {
        logDebug(DDSRECORDER, "Reloading DDS Recorder configuration...");
        ...
        {
            logDebug(DDSRECORDER, "Same configuration, do nothing in reload.");
        ...
        logDebug(DDSRECORDER, "New DDS Recorder allowed topics configuration: " << allowed_topics_);
...
utils::ReturnCode DDSRouterImpl::start() noexcept
{
    ...
    {
        logInfo(DDSRECORDER, "Starting DDS Recorder.");
    }
    else if (ret == utils::ReturnCode::RETCODE_PRECONDITION_NOT_MET)
    {
        logInfo(DDSRECORDER, "Trying to start an enabled DDS Recorder.");
    }
...
utils::ReturnCode DDSRouterImpl::stop() noexcept
{
    ...
    if (ret == utils::ReturnCode::RETCODE_OK)
    {
        logInfo(DDSRECORDER, "Stopping DDS Recorder.");
    }
    else if (ret == utils::ReturnCode::RETCODE_PRECONDITION_NOT_MET)
    {
        logInfo(DDSRECORDER, "Trying to stop a not enabled DDS Recorder.");
    }
...
utils::ReturnCode DDSRouterImpl::start_() noexcept
{
    ...
    {
        logInfo(DDSRECORDER, "Trying to start an already enabled DDS Recorder.");
...
utils::ReturnCode DDSRouterImpl::stop_() noexcept
{
    ...
    {
        logInfo(DDSRECORDER, "Trying to stop a disabled DDS Recorder.");
        return utils::ReturnCode::RETCODE_PRECONDITION_NOT_MET;
    }
}
...
void DDSRouterImpl::init_allowed_topics_()
{
    ...
    logInfo(DDSRECORDER, "DDS Recorder configured with allowed topics: " << allowed_topics_);
}
...
void DDSRouterImpl::init_participants_()
{
            ...
            configuration_.participants_configurations)
    {
        ...
        logInfo(DDSRECORDER, "Participant created with id: " << new_participant->id()
    ...
    // If DDS Recorder has not two or more Participants configured, it should fail
    if (participants_database_->size() < 1)
    {
        logError(DDSRECORDER, "At least a Participant is required to initialize a DDS Recorder.");
        throw utils::InitializationException(utils::Formatter()
                      << "DDS Recorder requires at least 1 Participant to start.");
    }
}
        ...
        const DdsTopic& topic) noexcept
{
    ...
    logInfo(DDSRECORDER, "Discovered topic: " << topic << ".");
        ...
        const GuidPrefix& server_guid_prefix) noexcept
{
    ...
    logInfo(DDSRECORDER, "Discovered service: " << topic << ".");
        ...
        const GuidPrefix& server_guid_prefix) noexcept
{
    ...
    logInfo(DDSRECORDER, "Removed service: " << topic << ".");
...
{
    logDebug(DDSRECORDER, "Endpoint discovered in DDS Recorder core: " << endpoint << ".");
...
void DDSRouterImpl::removed_endpoint_(
        const Endpoint& endpoint) noexcept
{
    logDebug(DDSRECORDER, "Endpoint removed/dropped: " << endpoint << ".");
        ...
        bool enabled /*= false*/) noexcept
{
    ...
    logInfo(DDSRECORDER, "Creating Bridge for topic: " << topic << ".");
        ...
        const RPCTopic& topic) noexcept
{
    ...
    logInfo(DDSRECORDER, "Creating Service: " << topic << ".");
        ...
        const DdsTopic& topic) noexcept
{
    ...
    logInfo(DDSRECORDER, "Activating topic: " << topic << ".");
        ...
        const DdsTopic& topic) noexcept
{
    ...
    logInfo(DDSRECORDER, "Deactivating topic: " << topic << ".");
