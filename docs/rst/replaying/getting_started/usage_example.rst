...
The only configuration option required by a |ddsreplayer| is the path to an input file, which can be provided both as a CLI argument or via YAML configuration.
Both the MCAP files and the SQL databases written by the |ddsrecorder| can be played back.
...
For example, you may try to modify the playback rate, block/allow the ``Square`` topic in the middle of execution, or set a different topic QoS configuration via the :ref:`Manual Topics <replayer_manual_topics>` list.
