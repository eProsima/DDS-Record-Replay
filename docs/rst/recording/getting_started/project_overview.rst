...
The recording can be written as an MCAP file, as an SQL database, or as both at the same time (see :ref:`Output Selection <recorder_usage_configuration_output_selection>`).
Furthermore, by using the `DynamicTypes <https://fast-dds.docs.eprosima.com/en/latest/fastdds/dynamic_types/dynamic_types.html>`_ feature of *Fast DDS*, it is possible to record the type of the data along with it.
