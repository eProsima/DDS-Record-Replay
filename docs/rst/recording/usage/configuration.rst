...
This *.yaml* file contains all the information regarding the DDS interface configuration, recording parameters, and |ddsrecorder| specifiations.
...
This way, a DDS Recorder is working in pause mode, recording samples in the database only when the event from the remote controller occurs.
