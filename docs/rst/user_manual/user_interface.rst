...
.. code-block:: console
    ...
    -d --debug          Set log verbosity to Info (Using this option with --log-filter and/or --log-verbosity will head to undefined behaviour).
        --log-filter     Set a Regex Filter to filter by category the info and warning log entries. [Default = "DDSROUTER|RECORDER"].
...
This argument enables the |ddsrecorder| logs so the execution can be followed by internal debugging information.
This argument sets :ref:`user_manual_user_interface_log_verbosity_argument` to ``info`` and
:ref:`user_manual_user_interface_log_filter_argument` to ``DDSRECORDER``.
...
A |ddsrecorder| **requires** one and only one *YAML* configuration file as the operation of this application is
...
This *YAML* configuration file must be passed as argument to the |ddsrecorder| when executed.
If no configuration file is provided as argument, the |ddsrecorder| will attempt to load a file named
``DDS_RECORDER_CONFIGURATION.yaml`` that must be in the same directory where the application is executed.
...
The topics that the |ddsrecorder| is routing could be changed at runtime.
Including topics in configuration's ``allowlist`` will create new :term:`Writers <DataWriter>` and
:term:`Readers <DataReader>` for each Participant in the Recorder.
...
So, if a topic has been active before, the Writers and Readers will still be present in the |ddsrecorder| and will still
receive data.

There exist two methods to reload the list of allowed topics, an active and a passive one.
Both methods work over the same configuration file with which the |ddsrecorder| has been initialized.


File Watcher
^^^^^^^^^^^^

A File Watcher is a process that runs in the background and watches for changes in the |ddsrecorder| configuration file.
Every time the file is changed, the OS sends a notification, and the File Watcher listens such notification
and interacts with the |ddsrecorder| in order to reload the topics.
This event occurs every time the configuration file is saved.

FileWatcher is used in every |ddsrecorder| execution by default.
...
Log module of |ddsrecorder| uses the |fastdds| logging module.
...
.. note::
    ...
    For ``INFO`` logs to be compiled, the |ddsrecorder| must have been compiled with CMake option ``CMAKE_BUILD_TYPE=Debug``,
    or compiled with CMake option ``LOG_INFO=ON``.

    If Fast DDS has been compiled in debug mode, it will print the logs of the DDS Recorder and Fast DDS mixed.
...
In order to stop a |ddsrecorder| application, use one of the following OS signals:
